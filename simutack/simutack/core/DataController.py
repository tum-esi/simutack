...
from simutack.sensor.IMUSensor import IMUSensor
from simutack.sensor.GNSSSensor import GNSSSensor
from simutack.sensor.CameraSensor import CameraSensor
from simutack.sensor.LidarSensor import LidarSensor
from simutack.sensor.RadarSensor import RadarSensor
from simutack.sensor.TachometerSensor import TachometerSensor
...
class DataController:
    ...
    def __init__(self, controller: Controller, carla_ip="localhost", carla_port=2000):
        """Constructor"""
    ...
    def init_carla(self):
        ...
        self.carla_client.set_timeout(30.0)  # 30s

        # Get world instance
        try:
            # self.carla_world = self.carla_client.load_world('Town03')
            self.carla_world = self.carla_client.load_world('Town02')
        ...
        # Set spectator to vehicle position
        # spectator = self.carla_world.get_spectator()
        # self.carla_world.wait_for_tick()
        # spectator.set_transform(self.carla_vehicle.get_transform())
    ...
    def create_sensor(self, sensor_type, name, update_interval=1.0):
        ...
        #     sensor = LidarSensor(self, name, update_interval)
        # elif sensor_type == 'radar':
        #     sensor = RadarSensor(self, name, update_interval)
        else:
    ...
    def reset_simulation(self, vehicle_transform):
        ...
        if self.carla_vehicle:
            if vehicle_transform:
                self.carla_vehicle.set_transform(vehicle_transform)
            else:
